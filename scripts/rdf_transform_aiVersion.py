...
# Graph
graph_uri = URIRef('http://example.org/imdb/graph')
dataset = Dataset()
...
for file in files:
    ...
    try:
        df = pd.read_csv(
            ...
            sep="\t",
            compression="gzip",
            on_bad_lines="skip"  # ignoriert defekte Zeilen
        )
        data_dict[file] = df
...
# -------- Title (basics) --------
df = data_dict["short.title.basics"]
for _, row in tqdm(df.iterrows(), total=len(df)):
    ...
    if not pd.isna(v) and str(v) not in {r'\N', '\\N', ''}:
        try:
            graph.add((t, IMD.startYear, Literal(str(int(v)), datatype=XSD.gYear)))
        except:
            pass
    ...
    if not pd.isna(v) and str(v) not in {r'\N', '\\N', ''}:
        try:
            graph.add((t, IMD.endYear, Literal(str(int(v)), datatype=XSD.gYear)))
        except:
            pass
...
# -------- Ratings --------
df = data_dict["short.title.ratings"]
for _, row in tqdm(df.iterrows(), total=len(df)):
    tconst = row["tconst"]
...
# -------- Episodes --------
df = data_dict["short.title.episode"]
for _, row in tqdm(df.iterrows(), total=len(df)):
    tconst = row["tconst"]
    t = URIRef(to_iri(f"{RES}title/{tconst}"))
    graph.add((t, RDF.type, IMD.Episode))
...
for _, row in tqdm(df.iterrows(), total=len(df)):
    ...
    if not pd.isna(v) and str(v) not in {r'\N', '\\N', ''}:
        try:
            graph.add((p, IMD.birthYear, Literal(str(int(v)), datatype=XSD.gYear)))
        except:
            pass
    ...
    if not pd.isna(v) and str(v) not in {r'\N', '\\N', ''}:
        try:
            graph.add((p, IMD.deathYear, Literal(str(int(v)), datatype=XSD.gYear)))
        except:
            pass
...
df = data_dict["short.title.principals"]
for _, row in tqdm(df.iterrows(), total=len(df)):
